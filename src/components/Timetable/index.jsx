...
const Timetable = memo(({ selectedTimes }) => {
	// 요일 배열
	const days = ['월요일', '화요일', '수요일', '목요일', '금요일'];

	const isSelectedTime = (day, time) => {
		console.log(time);
		return selectedTimes.some(
			selectedTime =>
				selectedTime.selectedDay === day && selectedTime.startTime === time
		);
	};
						...
						<tr key={index}>
							...
							{days.map(day => (
								<s.StyledTd
									key={day}
									style={{
										backgroundColor: isSelectedTime(day, classTime.startTime)
											? 'var(--blue-mute)'
											: '#fff',
									}}
								/>
